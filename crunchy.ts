...
export default class Crunchy implements ServiceClass {
	...
	public async listSeriesID(
		...
		for (const key of Object.keys(sortedEpisodes)) {
			...
			const seasonTitle = item.items.find((a) => !a.season_title.match(/\(\w+ Dub\)/))?.season_title ?? item.items[0].season_title.replace(/\(\w+ Dub\)/g, '').trimEnd();
			const dubs = item.items
				.map((a, index) => {
			...
			console.info([header, `\t- Dubs: ${dubs}`, `\t- Subs: ${subs}`].join('\n'));
		}
