...
export default class Crunchy implements ServiceClass {
  ...
  public async cli() {
    ...
    if (argv.silentAuth && !argv.auth) {
      ...
      await this.logSeriesById(argv.series as string);
			...
			if (RawOutputManager.shouldOutputRaw(argv)) {
				const allFlag = argv.all || (!argv.e && !argv.but);
				const rawSelected = await this.downloadFromSeriesID(argv.series, { ...argv, all: allFlag, forRawExport: true });
				await RawOutputManager.saveRawOutput({
					service: 'crunchy',
					data: rawSelected,
					outputPath: RawOutputManager.getOutputPath(argv),
  ...
  public async muxStreams(data: DownloadedMedia[], options: CrunchyMuxOptions) {
	...
	public async downloadFromSeriesID(id: string, data: CrunchyMultiDownload & { forRawExport?: boolean }): Promise<ResponseBase<CrunchyEpMeta[]>> {
    const { data: episodes } = await this.listSeriesID(id, data);
    console.info('');
    console.info('-'.repeat(30));
    console.info('');
		const selected = this.itemSelectMultiDub(episodes, data.dubLang, data.but, data.all, data.e, data.absolute, data.forRawExport);
        ...
        if (item.streams_link) {
          epMeta.data[0].playback = item.streams_link;
            ...
            item.playback = item.streams_link;
				...
				if ((forRawExport || item.playback) && ((but && !doEpsFilter.isSelected([epNum, item.id])) || all || (doEpsFilter.isSelected([epNum, item.id]) && !but))) {
