      ...
      if (options.tsd) {
        ...
        const activeStreamsReq = await this.req.getData(api.streaming_sessions, AuthHeaders);
          ...
          console.warn(`Killed ${data.items?.length ?? 0} Sessions`);
			...
			let videoPlaybackReq = await this.req.getData(
				`https://www.crunchyroll.com/playback/v3/${currentVersion ? currentVersion.guid : currentMediaId}/${CrunchyVideoPlayStreams[options.vstream]}/play?queue=0`,
				AuthHeaders
			);
        ...
        if (isDLVideoBypass) {
					// Try old download endpoint first
					let videoDLReq = await this.req.getData(
					...
					// If we got static manifest, try cr-play-service for evs3
					if (videoDLReq.ok && videoDLReq.res) {
						const dlData = (await videoDLReq.res.json()) as CrunchyPlayStream;
						if (dlData.url && dlData.url.includes('/static/')) {
							const crPlayServiceReq = await this.req.getData(
								`https://cr-play-service.prd.crunchyrollsvc.com/v1/${currentVersion ? currentVersion.guid : currentMediaId}/tv/android_tv/play`,
								AuthHeaders
							);
							if (crPlayServiceReq.ok && crPlayServiceReq.res) {
								const crPlayServiceStream = (await crPlayServiceReq.res.json()) as CrunchyPlayStream;
								if (crPlayServiceStream.url && crPlayServiceStream.url.includes('/evs3/')) {
									videoDLReq = crPlayServiceReq;
								}
							}
						}
					}
        ...
        pbData.vpb[`adaptive_${options.vstream}_${videoStream.url.includes('m3u8') ? 'hls' : 'dash'}_drm`] = {
          ...derivedPlaystreams
			...
			if (!options.cstream && options.vstream !== options.astream && videoStream) {
				// Try old endpoint first
				let audioPlaybackReq = await this.req.getData(
        ...
        if (!audioPlaybackReq.ok || !audioPlaybackReq.res) {
          console.warn('Request Audio Stream URLs FAILED!');
					...
					// If video stream uses evs3, use same for audio to match
					if (videoStream.url && videoStream.url.includes('/evs3/')) {
						const crPlayServiceReq = await this.req.getData(
							`https://cr-play-service.prd.crunchyrollsvc.com/v1/${currentVersion ? currentVersion.guid : currentMediaId}/tv/android_tv/play`,
							AuthHeaders
						);
						if (crPlayServiceReq.ok && crPlayServiceReq.res) {
							const crPlayServiceStream = (await crPlayServiceReq.res.json()) as CrunchyPlayStream;
							if (crPlayServiceStream.url && crPlayServiceStream.url.includes('/evs3/')) {
								audioStream = crPlayServiceStream;
							}
						}
					}
          const derivedPlaystreams = {} as CrunchyStreams;
      ...
      if (options.novids && options.noaudio) {
        ...
        const vstreamPlaylistsReq = await this.req.getData(vcurStream.url, AuthHeaders);
          ...
          if (vstreamPlaylistBody.match('MPD') && astreamPlaylistBody.match('MPD')) {
            ...
            //set Video Qualities
						let videos = vselectedList.video.map((item) => {
              return {
                ...item,
								resolutionText: `${item.quality.width}x${item.quality.height} (${Math.round(item.bandwidth / 1024)}KiB/s)`
              };
						...
						let audios = aselectedList.audio.map((item) => {
              return {
                ...item,
								resolutionText: `${Math.round(item.bandwidth / 1000)}kB/s`
              };
      ...
      if (options.nosubs) {
        ...
        options.skipsubs = true;
          ...
          const capsData = Object.values(pbData.meta.closed_captions);
					...
					// Check if CC subtitles are missing when using static manifest
					if (vcurStream && vcurStream.url.includes('/static/') && capsData.length === 0 && options.dlsubs.some(s => s.toLowerCase() === 'en' || s.toLowerCase() === 'all')) {
						console.warn(
							'[WARN] Closed captions (CC) not available from static manifest. CC subtitles may be available via evs3 endpoint, but this is a Crunchyroll API limitation for this episode.'
						);
					}
